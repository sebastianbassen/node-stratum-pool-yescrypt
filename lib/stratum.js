...
var StratumClient = function (options) {
    ...
    function handleAuthorize(message, replyToSocket) {
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function (result) {
            ...
            if (_this.workerPass.startsWith("diff=")) {
                var customDiffString = _this.workerPass.substring(5);
                if (customDiffString.match(/^-{0,1}\d+$/) || num.match(/^\d+\.\d+$/)) {
                    _this.customDiff = parseFloat(customDiffString);
                    console.log("custom diff set!");
                }
            }
            ...
            _this.sendDifficulty(_this.customDiff);
    ...
    this.enqueueNextDifficulty = function (requestedNewDifficulty) {
        if (_this.customDiff != null) {
            pendingDifficulty = _this.customDiff;
        } else {
